...
const SERVICES = ['Architecture','Interior Design','Web Development','Software / Platform','Brand Strategy','Identity Design','Campaign Planning','Performance Marketing'];
...
export default function ContactPage() {
  ...
  const submit = async (e: FormEvent) => {
    e.preventDefault();
    setStatus('sending');
    const payload = { name, email, company, services, budget, timeline, message, source: 'website', page_url: window.location.href, user_agent: navigator.userAgent };
    try {
      ...
      <section style={{ paddingTop: '8rem', paddingBottom: '6rem' }}>
        <div className="container">
          <div className="contact-grid">
            {/* Left */}
            <div>
              <div className="section-label"><span className="num" style={{ color: 'var(--azure)' }}>Contact</span><span className="line" /></div>
              ...
              <div className="contact-meta">
                ...
                <div className="row"><span className="k">HQ Address</span><span className="v" style={{ lineHeight: 1.5 }}>1942 Broadway St., STE 314C<br/>Boulder, Colorado 80302</span></div>
              </div>
            </div>
            ...
            <div>
              {status === 'ok' ? (
                <div className="form-status">
                  ...
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'var(--paper-dim)' }}>We&apos;ll be in touch within one business day.</p>
                </div>
              ) : (
                <form onSubmit={submit} noValidate>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', marginBottom: '2.5rem' }}>
                    {([1,2,3] as Step[]).map(s => (
                      <button key={s} type="button" onClick={() => step > s && setStep(s)}
                        style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: step === s ? 'var(--azure)' : step > s ? 'var(--paper-dim)' : 'var(--paper-mute)', paddingBottom: '0.5rem', borderBottom: `1px solid ${step === s ? 'var(--azure)' : 'transparent'}`, transition: 'all 0.3s var(--ease-out)', cursor: step > s ? 'pointer' : 'default' }}>
                        {s === 1 ? '01 About You' : s === 2 ? '02 The Project' : '03 The Brief'}
                      </button>
                    ))}
                  </div>

                  {step === 1 && (
                    <div className="form" style={{ gap: '1.5rem' }}>
                      ...
                      <button type="button" className="btn btn-primary" onClick={() => { if (name && email) setStep(2); }} style={{ marginTop: '0.5rem', alignSelf: 'flex-start' }}>
                  ...
                  {step === 2 && (
                    <div className="form">
                      ...
                      <button type="button" className="btn btn-primary" onClick={() => setStep(3)} style={{ marginTop: '0.5rem', alignSelf: 'flex-start' }}>
                        Continue <span className="arrow">→</span>
                      </button>
                    </div>
                  )}
                  ...
                  {step === 3 && (
                    <div className="form">
                      <div className="field">
                        ...
                        <textarea id="message" value={message} onChange={e => setMessage(e.target.value)} required rows={7} placeholder="Tell us about your project. What are you building, what problem does it solve, and what does success look like?"/>
                      </div>
                      {status === 'error' && (
                      ...
                      <button type="submit" className="btn btn-primary" disabled={status === 'sending'} style={{ marginTop: '0.5rem', alignSelf: 'flex-start', opacity: status === 'sending' ? 0.6 : 1 }}>
